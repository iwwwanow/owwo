...
- [ ] - составить список роутов, котороые вообще будут на сайте
- [ ] - прописать их все с заглушками
- [ ] - вынести хендлеры в персональные сервисы

# 29.12

- [ ] - расширить тип. добавить в него params
- [ ] - проверить вхождение этих params
- [ ] - в целом, не нравится как это выглядит. но если это будет работать, то лучше оставить и вернуться потом

# 28.12

- почему ты функционал своего адаптера подбиваешь под интерфесы какогото фреймворка?
- это должно быть просто как молоток
- мне нужен просто регекс, который словит pathname
- slug можно вытащить прямо из pathname в функции-хендлере
- это будет самый оптимальнй вариант, чем городить чтото похожее на чтото

- [ ] - изменить метод с get на addRouteHander(route: string | regex, handler: (req) => Promise<Response>)

- [ ] - если учесть то, что на данном этапе роутов, сложнее /node/:nodeId не будет, может быть ограничиться функционалом для таких роутов?
